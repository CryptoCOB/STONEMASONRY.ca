      ...
      <div className="container-custom">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {filteredItems.map((item, index) => (
            <motion.div
              key={item.id}
              className="overflow-hidden bg-white rounded-lg shadow-lg group hover:shadow-xl transition-all duration-300"
            ...
            >
              <div className="relative h-64 overflow-hidden">
                ...
                <button
                  ...
                  className="absolute top-4 right-4 bg-black/70 text-white px-3 py-1 rounded-full text-xs font-medium hover:bg-black/90 transition-colors"
                >
                  {showBeforeAfter[item.id] ? 'Show Before' : 'Show After'}
                </button>

                {/* Category Badge */}
                <div className="absolute top-4 left-4 bg-primaryRed text-white px-3 py-1 rounded-full text-xs font-medium">
                  {item.category}
                </div>

                {/* Before/After Label */}
                <div className="absolute bottom-4 left-4 bg-white/90 text-gray-800 px-3 py-1 rounded-full text-xs font-medium">
        ...
        >
          <button
            ...
            className="text-lg btn-primary hover:bg-red-700 transition-colors"
          >
